...
const componentSpecs = [ {
  id: "cpu",
  label: "CPU",
  detail: "16 CORE / 5.4 GHZ",
  type: "core"
}, {
  id: "gpu",
  label: "GPU",
  detail: "PCIe x16 / ACTIVE",
  type: "large"
}, {
  id: "ram",
  label: "RAM",
  detail: "DDR5 / 6400 MT/s",
  type: "memory"
}, {
  id: "nvme",
  label: "NVMe",
  detail: "PCIe 5.0 / 7.2 GB/s",
  type: "drive"
}, {
  id: "power",
  label: "POWER",
  ...
  id: "io",
  label: "I/O",
  detail: "USB4 / LAN / AUDIO",
  type: "small"
}, {
  id: "network",
  label: "NETWORK",
  detail: "2.5 GbE / WI-FI 7",
  type: "network"
}, ];
...
function layoutComponents() {
  ...
  const horizontal = width < 720 ? Math.min(width * 0.46, 220) : Math.min(width * 0.31, 480);
  const vertical = width < 720 ? Math.min(height * 0.31, 235) : Math.min(height * 0.42, 365);
  const positions = {
...
function buildRoutes() {
  ...
  for (const component of components) {
    ...
    for (let lane = 0; lane < config.lanes; lane += 1) {
      const laneOffset = (lane - (config.lanes - 1) / 2) * 5.5 * portScale;
      const start = portOnSide(cpu, config.sourceSide, config.sourceBase * portScale + laneOffset, );
...
function portOnSide(component, side, offset = 0) {
  ...
  if (side === "left" || side === "right") {
    return {
      ...
      y: component.y + Math.max(-halfHeight + 7, Math.min(halfHeight - 7, offset)),
    };
  }
  return {
    x: component.x + Math.max(-halfWidth + 7, Math.min(halfWidth - 7, offset)),
...
function drawRoute(route) {
  const active = hoveredComponent === route.component || hoveredComponent?.id === "cpu";
...
function drawPackets() {
  ...
  for (const packet of packets) {
    ...
    const point = pointOnRoute(packet.route, packet.progress);
    const active = hoveredComponent === packet.route.component || hoveredComponent?.id === "cpu";
...
function drawPins(component) {
  ...
  for (let offset = -component.width / 2 + spacing; offset < component.width / 2; offset += spacing ) {
  ...
  for (let offset = -component.height / 2 + spacing; offset < component.height / 2; offset += spacing ) {
...
function drawComponent(component) {
  ...
  if (component.type === "core") {
    const inset = 12;
    roundedRect(x + inset, y + inset, component.width - inset * 2, component.height - inset * 2, 4, );
...
function updateHover() {
  ...
  for (const component of components) {
    const inside = Math.abs(pointer.x - component.x) < component.width / 2 + 12 && Math.abs(pointer.y - component.y) < component.height / 2 + 12;
...
for (const metric of metricElements) {
  ...
  sparkline.innerHTML = Array.from( {
    length: 18
  }, () => "<i></i>").join("");
}
function updateTelemetry() {
  for (const metric of metricElements) {
    ...
    const wave = Math.sin(time * 0.021 + config.phase) * 0.6 + Math.sin(time * 0.047 + config.phase * 2) * 0.4;
    const boost = (component?.pulse || 0) * 28;
    const usage = Math.round(Math.max(8, Math.min(99, config.base + wave * config.range + boost)), );
    metric.querySelector("strong").textContent = `${usage}%`;
    const bars = metric.querySelectorAll(".sparkline i");
    bars.forEach((bar, index) => {
      if (index < bars.length - 1) {
        bar.style.transform = bars[index + 1].style.transform || "scaleY(0.3)";
      }
      else {
        const value = Math.max(0.1, Math.min(1, (usage + (Math.random() - 0.5) * 14) / 100), );
...
function animate() {
  ctx.drawImage(boardLayer, 0, 0, boardLayer.width, boardLayer.height, 0, 0, width, height, );
