...
function resize() {
  ...
  const targetCount = Math.min(
    220,
    Math.max(80, Math.round((width * height) / settings.particleDensity)),
  );
...
function applyGravity(particle) {
  ...
  particle.vx +=
    (pointer.x - pointer.previousX) * proximity * pointer.speed * 0.0008;
  particle.vy +=
    (pointer.y - pointer.previousY) * proximity * pointer.speed * 0.0008;
