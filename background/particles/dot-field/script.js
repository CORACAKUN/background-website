...
function updateDot(dot) {
  ...
  if (pointer.active && distance < settings.radius) {
    ...
    dot.vx += (dx / distance) * easedForce * settings.strength * 0.16 * direction;
    dot.vy += (dy / distance) * easedForce * settings.strength * 0.16 * direction;
  }
...
function drawDot(dot) {
  ...
  const proximity = pointer.active ? Math.max(0, 1 - pointerDistance / (settings.radius * 1.25)) : 0;
