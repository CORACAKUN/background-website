...
function waveHeight(x, y) {
  let heightValue = Math.sin(x * 0.012 + time * 0.018) * 2.8 + Math.cos(y * 0.014 - time * 0.015) * 2.2;
  if (pointer.active) {
    const distance = Math.hypot(x - pointer.x, y - pointer.y);
    if (distance < settings.hoverRadius) {
      const proximity = 1 - distance / settings.hoverRadius;
      heightValue += Math.cos(distance * 0.045 - time * 0.085) * proximity * 19 * pointer.energy;
    }
  }
  for (const ripple of ripples) {
    ...
    if (distanceFromRing < bandWidth) {
      ...
      heightValue += Math.sin((distance - ripple.radius) * 0.07) * band * ripple.life * ripple.strength * 27;
    }
  }
  return heightValue;
}
