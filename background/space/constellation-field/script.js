...
function createParticle() {
  ...
  return {
    ...
    radius: randomBetween(0.65, 1.45) * depth, depth,
    phase: Math.random() * Math.PI * 2,
  };
}
function setParticleCount() {
  particleCount = Math.min(115, Math.max(42, Math.round((width * height) / config.density)), );
...
function drawConnection(a, b, distance, maximum, cursorConnection = false) {
  ...
  gradient.addColorStop(0, `rgba(${cursorConnection ? "127, 231, 255" : "113, 129, 225"}, ${opacity})`, );
...
function drawCursorNode() {
  ...
  const glow = ctx.createRadialGradient(pointer.x, pointer.y, 0, pointer.x, pointer.y, 19, );
  ...
  for (let i = 0; i < particles.length; i += 1) {
    ...
    for (let j = i + 1; j < particles.length; j += 1) {
      ...
      if (distance < config.connectionDistance) {
        drawConnection(particle, neighbor, distance, config.connectionDistance, );
      }
    }
    if (pointer.active) {
      const cursorDistance = Math.hypot(particle.x - pointer.x, particle.y - pointer.y, );
      if (cursorDistance < config.cursorDistance) {
        drawConnection(particle, pointer, cursorDistance, config.cursorDistance, true, );
      }
    }
