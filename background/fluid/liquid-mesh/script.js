...
const blobs = [];
const palette = [[29, 170, 132], [62, 111, 157], [124, 74, 161], [181, 98, 102], [181, 211, 76], [28, 132, 117], ];
let width = 0;
let height = 0;
let dpr = 1;
let time = 0;
...
function updateBlob(blob, index) {
  ...
  const flowY = Math.sin(time * blob.speed * 1.17 + blob.phase) * blob.orbitY;
  ...
  if (pointer.active || pointer.influence > 0.01) {
    ...
    if (distance < reach) {
      ...
      blob.vx += (pointer.x - pointer.previousX) * proximity * 0.0018 * pointer.influence;
      blob.vy += (pointer.y - pointer.previousY) * proximity * 0.0018 * pointer.influence;
    }
  }
  blob.vx *= 0.965;
  blob.vy *= 0.965;
...
function drawBlob(blob) {
  ...
  const gradient = ctx.createRadialGradient(blob.x, blob.y, blob.radius * 0.04, blob.x, blob.y, blob.radius, );
...
function drawCursorCurrent() {
  ...
  gradient.addColorStop(0, `rgba(208, 255, 126, ${0.15 * pointer.influence})`, );
  gradient.addColorStop(0.42, `rgba(80, 235, 196, ${0.08 * pointer.influence})`, );
